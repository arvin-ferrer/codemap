...
describe('TypeScriptAstExtractorService', () => {
  let service: TypeScriptAstExtractorService;
  let testRoot: string;
  let mockNodes: CodeNode[];

  beforeAll(() => {
    // Create temporary workspace structure
    testRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'codemap-ast-test-'));
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [TypeScriptAstExtractorService],
    }).compile();

    service = module.get<TypeScriptAstExtractorService>(
      TypeScriptAstExtractorService,
    );
  });
  ...
  describe('parse', () => {
    it('should extract AST and resolve imports into dependency links', async () => {
      const links = await service.parse(testRoot, mockNodes);

      // Expected links:
      // 1. src/index.ts ➔ src/helper.ts (static-import)
      // 2. src/index.ts ➔ src/utils/logger.js (static-import/require)
      // 3. src/index.ts ➔ src/components/Button/index.tsx (static-import)
      // Ignored:
      ...
      expect(linkLogger?.relation).toBe('static-import');
