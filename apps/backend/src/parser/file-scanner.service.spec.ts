...
describe('FileScannerService', () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(path.join(testRoot, 'src', 'index.ts'), 'console.log("TS");\nconsole.log("Line 2");');
    fs.writeFileSync(path.join(testRoot, 'src', 'utils.js'), 'module.exports = {};');
    ...
    fs.writeFileSync(path.join(testRoot, 'node_modules', 'helper.js'), 'module.exports = {};');
    ...
    fs.writeFileSync(path.join(testRoot, 'ignored-folder', 'temp.js'), 'console.log("temp");');
    fs.writeFileSync(path.join(testRoot, 'src', 'cache.json'), '{}');

    // Create .gitignore
    fs.writeFileSync(path.join(testRoot, '.gitignore'), `
# Comments
ignored-folder/
src/cache.json
    `);
  ...
  describe('scan', () => {
    it('should recursively scan whitelisted files and gather correct metadata', () => {
      ...
      const indexNode = nodes.find(n => n.id === path.join('src', 'index.ts').replace(/\\/g, '/'));
      ...
      const utilsNode = nodes.find(n => n.id === path.join('src', 'utils.js').replace(/\\/g, '/'));
