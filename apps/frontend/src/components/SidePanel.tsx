...
export default function SidePanel({ nodeId, nodes, links, onClose }: SidePanelProps) {
    ...
    <div className={styles.panel}>
      <div className={styles.header}>
        ...
        <div className={styles.stats}>
          ...
          <span className={styles.badge}>{(node.size / 1024).toFixed(1)} KB</span>
        </div>
      </div>
      ...
      <div className={styles.content}>
        <div className={styles.section}>
          ...
          <ul className={styles.list}>
            {outgoing.map(n => (
              <li key={n.id} className={styles.listItem}>{n.name}</li>
            ))}
        ...
        <div className={styles.section}>
          <h3 className={styles.sectionTitle}>Imported By ({incoming.length})</h3>
          <ul className={styles.list}>
            {incoming.map(n => (
              <li key={n.id} className={styles.listItem}>{n.name}</li>
            ))}
